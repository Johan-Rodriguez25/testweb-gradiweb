  ...
  getProducts.onload = () => {
    if (getProducts.status === 200) {
      ...
      document.getElementById(
        'product-rate'
      ).innerHTML = `${products[0].rating.rate} ⭐`
      document.getElementById(
        'product-count'
      ).innerHTML = `(${products[0].rating.count})`
      ...
      document.getElementById(
        'product-rate2'
      ).innerHTML = `${products[14].rating.rate} ⭐`
      document.getElementById(
        'product-count2'
      ).innerHTML = `(${products[14].rating.count})`
...
export function showProduct(number) {
  ...
  document.getElementById(
    'product-rate'
  ).innerHTML = `${products[number].rating.rate} ⭐`
  document.getElementById(
    'product-count'
  ).innerHTML = `(${products[number].rating.count})`
}
