...
export function queries() {
  ...
  convertUsdToEur.open(
    'GET',
    'https://api.getgeoapi.com/v2/currency/convert?api_key=d1fd585c6ce57d2b390d05674c87b45b8b78b5ab&from=USD&to=EUR&amount=1&format=json',
    true
  )
  convertUsdToEur.send()

  let convertEurToUsd = new XMLHttpRequest()
  convertEurToUsd.open(
    'GET',
    'https://api.getgeoapi.com/v2/currency/convert?api_key=d1fd585c6ce57d2b390d05674c87b45b8b78b5ab&from=EUR&to=USD&amount=1&format=json',
    true
  )
...
export function changeCurrency(number) {
  ...
  if (!isUSDSelected) {
    ...
    priceConverted = (
      document.getElementById('product-price').innerHTML * usdToEur
    ).toFixed(2)
    ...
    priceConverted = (
      document.getElementById('product-price').innerHTML * eurToUsd
    ).toFixed(2)
    document.getElementById('product-price').innerHTML = priceConverted
  }
}
